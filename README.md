...
Enterprise Asset & Resource Management System. backend/ = FastAPI + SQLAlchemy + PostgreSQL (untouched functionality). frontend/ = React (TanStack Start + Tailwind + shadcn) — fully rewired from Supabase to the FastAPI backend via frontend/src/lib/api.ts (JWT auth, auto token refresh, typed endpoint helpers).
